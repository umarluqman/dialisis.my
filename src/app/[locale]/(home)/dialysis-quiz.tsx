...
export function DialysisQuiz({ initialData }: { initialData: any }) {
  ...
  const formatOptionLabel = useCallback(
    (option: string) => {
      ...
      if (lower === "moh") return tSector("moh");
      if (lower === "moh_private") return tSector("mohPrivate");
      if (lower === "private") return tSector("private");
      if (lower === "ngo") return tSector("ngo");
      if (lower === "university") return tSector("university");
      if (lower === "armed force") return tSector("armedForce");
      if (lower === "others") return tSector("others");

      if (lower.includes("tiada hepatitis")) return tHepatitis("none");
      if (lower.includes("hepatitis b")) return tHepatitis("b");
      if (lower.includes("hepatitis c")) return tHepatitis("c");

      if (lower.includes("hemodialisis")) return tTreatments("hd");
      if (lower.includes("peritoneal")) return tTreatments("pd");
      if (lower.includes("transplant")) return tTreatments("tx");
      if (lower.includes("mrrb")) return tTreatments("mrrb");

      return option;
    },
    [tSector, tHepatitis, tTreatments]
  );

  const steps: QuizStep[] = useMemo(
    () => [
      {
        question: t("questions.state"),
      ...
      {
        question: t("questions.treatment"),
        options: TREATMENT_TYPES,
        param: "treatment",
      },
      {
        question: t("questions.hepatitis"),
        options: HEPATITIS_OPTIONS,
        param: "hepatitis",
      },
      {
        question: t("questions.sector"),
        options: SECTOR,
        param: "sector",
      },
    ],
              ...
              <div className="mb-6">
                ...
                <div className="mt-2 space-y-1">
                  ...
                  <div className="flex flex-row gap-2 flex-wrap">
                    ...
                    {treatmentParam && getFormattedValue(treatmentParam) && (
                      <Badge className="bg-primary-foreground text-white rounded-none font-semibold text-sm">
                        {formatOptionLabel(getFormattedValue(treatmentParam)!)}
                      </Badge>
                    )}
                    {hepatitisParam && getFormattedValue(hepatitisParam) && (
                      <Badge className="bg-primary-foreground text-white rounded-none font-semibold text-sm">
                        {formatOptionLabel(getFormattedValue(hepatitisParam)!)}
                      </Badge>
                    )}
                    {sectorParam && getFormattedValue(sectorParam) && (
                      <Badge className="bg-primary-foreground text-white rounded-none font-semibold text-sm">
                        {formatOptionLabel(getFormattedValue(sectorParam)!)}
                      </Badge>
                    )}
                  </div>
                </div>
